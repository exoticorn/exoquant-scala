...
object Main extends App {
  ...
  val pixels = inputImage.getData().getPixels(0, 0, img.getWidth(), img.getHeight(), null: Array[Int])
  for {
    x <- 0 until img.getWidth()
    y <- 0 until img.getHeight()
  } {
    dataBuffer.getData()(x + y * img.getWidth()) = pixels((x + y * img.getWidth()) * 4).toByte
  }
